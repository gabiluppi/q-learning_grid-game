...
from config import NUM_AGENTS, CELL_SIZE, GAMMA, ALPHA, EPSILON, EPISODES, MOVE_DELAY, FPS, STEP_PENALTY
...
def setup():
    ...
    pygame.display.set_caption("V2: Multi-Agent Q-Learning (With Step Penalty)")
...
async def update_loop():
    ...
    for episode in range(EPISODES):
        ...
        while not all(grid[state[0]][state[1]] == 'G' for state in agent_states):
            ...
            for agent_idx in range(NUM_AGENTS):
                ...
                reward = rewards[grid[next_state[0]][next_state[1]]] + STEP_PENALTY
    ...
    for agent_idx in range(NUM_AGENTS):
        print(f"\nOptimal Policy for Agent {agent_idx + 1} (V2):")
